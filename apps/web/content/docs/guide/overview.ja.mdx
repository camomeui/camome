...
それに大して Camome UI の [`Button`](/docs/components/inputs/button)は単体で **1.42KB**(CSS 含む)と軽量であり、それにグローバル CSS の 4KB が加わります。
...
React ライブラリとしての使用を第一に考えられていますが、バンドル済みの CSS をインポートすることで、Bootstrap のような CSS フレームワークとしても使用できます。
...
このように、React のエコシステムに依存しているわけではないので、Vue や Svelte のような別のフレームワークに移植することも容易なはずです。
